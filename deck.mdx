...
- Mainly for I/O heavy tasks   
- Main Thread > async method > await > ThreadPool > Task Scheduler > allocate CPU time for the main thread (not blocking) >
- await > Waiting for I/O result > Set state machine to waiting > 
- I/O result ready > Scheduler allocate another thread
...
```csharp showNumbers subtitle="Async"
    ...
    //99500 bottles filled in 00:00:06.6724608 time when PotSizeInLiter=100
...
```diff 42:59 subtitle="Race condition issue"
```
...
```csharp showNumbers
    ...
    //100000 bottles filled in 00:00:06.6982469 time where PotSizeInLiter=100 with lock object
...
```diff 42:60 subtitle="Thread safe async with lock"
```
...
```csharp showNumbers subtitle="Multithreading"
    ...
    //99400 bottles filled in 00:00:04.7836539 time
...
```diff 38:53 subtitle="Race condition"
```
...
```csharp showNumbers 
    ...
    //100000 bottles filled in 00:00:04.8008058 time

```
```diff 40:66 subtitle="Foreground threads are not managed by ThreadPool"
```
...
```csharp showNumbers 
    ...
    private static void Main()
    {
        ...
        for (var start = 0; start < EmptyBottle; start += PotSizeInLiter * 2)
        {
          var brewTask = Task.Run(() => Brewing("IPA", PotSizeInLiter * 2));
          brewList.Add(brewTask);
        }
    ...
    //100000 bottles filled in 00:00:00.1891661 time

```

```diff 7:19 subtitle="Task.Run() force background threads"

```

```diff 48:56 subtitle="Background threads are managed by ThreadPool"
```
